...
def run_korea_sector(excel_bytes):
    # 사용자가 준 전체 국내 코드로 계산. 화면용 객체/그림은 저장하지 않고 숫자 결과만 축약.
    src = KR_SECTOR_SRC
    buf = io.StringIO()
    old_show = plt.show
    def _display(_obj):
    ...
    def _show(*args, **kwargs):
        try:
            ...
            pass
    plt.show = _show
    ns = {"display": _display, "EXCEL_BYTES": excel_bytes}
    try:
        with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(buf):
            exec(src, ns, ns)
        compact = _compact_korea_sector_namespace(ns)
        # 진단용 텍스트도 끝부분만 보관해 용량 최소화
        txt = buf.getvalue()[-12000:]
        return True, compact, txt, [], []
    except Exception:
        buf.write("\n" + traceback.format_exc())
        return False, {}, buf.getvalue()[-20000:], [], []
    finally:
        plt.show = old_show
        plt.close("all")


def _show_df(title, obj, *, hide_index=False, tail=None):
    if isinstance(obj, pd.DataFrame) and not obj.empty:
        st.markdown(f"### {title}")
        z = obj.tail(tail) if tail else obj
        st.dataframe(z, use_container_width=True, hide_index=hide_index)
        return True
    return False
...
def render_korea_sector_compact(ns):
    ...
    _show_df("Mansfield RS ≥ 70", ns.get("mansfield_df_70"), hide_index=True)
    _show_df("6개월 수익률 · KOSPI 대비 상대수익률", ns.get("six_month_summary"))

    mom = ns.get("momentum_df")
    if isinstance(mom, pd.DataFrame) and not mom.empty:
        st.markdown("### 6개월 변동성 조정 모멘텀 Top 20 · 이평선 조건 없음")
        st.caption("요청대로 변동성 조정 모멘텀은 이 표 하나만 표시합니다. 10일·20일·10주 이평선 필터 순위는 제외했습니다.")
        st.dataframe(mom.tail(10), use_container_width=True)

    st.markdown("### 6개월 Sortino Top 20")
    sortino_items = [
        ("10일선 이상", ns.get("sortino_ma10")),
        ("20일선 이상", ns.get("sortino_ma20")),
        ("10주선(50일) 이상", ns.get("sortino_ma10w")),
    ]
    for label, df in sortino_items:
    ...
    # 원본 코드의 ETR Comfort는 유지하되 기본 화면을 복잡하게 만들지 않도록 접음
    with st.expander("ETR Comfort Top 20 · 원본 코드 유지", expanded=False):
        for label, key in [("10일선 이상","etr_ma10"),("20일선 이상","etr_ma20"),("10주선(50일) 이상","etr_ma10w")]:
            st.markdown(f"**{label}**")
            df = ns.get(key)
            if isinstance(df, pd.DataFrame) and not df.empty:
                st.dataframe(df.tail(10), use_container_width=True)
    ...
    st.markdown("### 업종 쏠림지수")
    crowd = ns.get("crowding_series")
    c1, c2 = st.columns(2)
    with c1:
        if ns.get("latest_crowd") is not None:
            try: st.metric("현재 쏠림지수", f"{float(ns['latest_crowd']):.2f}")
            except Exception: pass
    with c2:
        if ns.get("signal") is not None:
            st.metric("판정", str(ns.get("signal")))
    if isinstance(crowd, pd.Series) and not crowd.empty:
        st.line_chart(crowd.rename("CrowdingIndex").tail(260), use_container_width=True)
    lc, rc = st.columns(2)
    with lc: _show_df("6개월 수익률 주도 업종 Top 10", ns.get("leaders_6m"))
    with rc: _show_df("6개월 수익률 소외 업종 Bottom 10", ns.get("laggards_6m"))

    st.markdown("### ETF/섹터 Breadth")
    bsum = ns.get("breadth_summary")
    if isinstance(bsum, pd.DataFrame):
        st.dataframe(bsum, use_container_width=True)
    cnt = ns.get("latest_count_table")
    if isinstance(cnt, pd.DataFrame):
        st.dataframe(cnt, use_container_width=True, hide_index=True)
    breadth = ns.get("sector_breadth")
    if isinstance(breadth, pd.DataFrame) and not breadth.empty:
        cols = [c for c in ["Above_MA10_Smooth","Above_MA20_Smooth","Above_MA50_Smooth","Above_MA120_Smooth"] if c in breadth.columns]
        if cols:
            st.caption("10일·20일·10주·120일선 위 업종 비율")
            st.line_chart(breadth[cols].tail(260), use_container_width=True)
        cols2 = [c for c in ["Breadth_Score","Breadth_Score_MA5"] if c in breadth.columns]
        if cols2:
            st.caption("종합 Breadth Score")
            st.line_chart(breadth[cols2].tail(260), use_container_width=True)
        cols3 = [c for c in ["Breadth_Slope_5D","Breadth_Slope_10D"] if c in breadth.columns]
        if cols3:
            st.caption("Breadth 기울기")
            st.line_chart(breadth[cols3].tail(260), use_container_width=True)

    if ns.get("latest_ma120_breadth") is not None:
        st.markdown("### 역추세용 MA120 Breadth")
        c1,c2,c3 = st.columns(3)
        with c1: st.metric("MA120 Breadth", f"{float(ns['latest_ma120_breadth']):.1f}%")
        with c2: st.metric("MA120 위 ETF", f"{int(ns.get('latest_ma120_count',0))}/{int(ns.get('latest_ma120_valid',0))}")
        with c3: st.metric("판정", str(ns.get("reverse_breadth_signal","")))

    gap_summary = ns.get("summary_df")
    if isinstance(gap_summary, pd.DataFrame) and not gap_summary.empty:
        st.markdown("### Sortino 10주선 이상 · 30일선 초과 이격도 / 247거래일 97% 구간")
        visible = [c for c in gap_summary.columns if not str(c).startswith("_")]
        st.dataframe(gap_summary[visible], use_container_width=True, hide_index=True)
        gap_data = ns.get("gap_chart_data") or {}
        for _, row in gap_summary.iterrows():
            sector = str(row.get("섹터"))
            z = gap_data.get(sector)
            if isinstance(z, pd.DataFrame) and not z.empty:
                rank = row.get("Sortino순위", "")
                with st.expander(f"#{rank} {sector} · 가격/MA30/Gap30", expanded=False):
                    st.line_chart(z[[c for c in ["Price","MA30"] if c in z.columns]], use_container_width=True)
                    if "Gap30" in z.columns:
                        st.line_chart(z[["Gap30"]], use_container_width=True)

    full = ns.get("full_alignment_df")
    ma = ns.get("ma_alignment_df")
    if isinstance(full, pd.DataFrame):
        _show_df("4주·13주·26주·52주 완전 정배열", full)
    if isinstance(ma, pd.DataFrame):
        _show_df("4주·13주·26주·52주 이동평균 정배열", ma)
    with st.expander("전체 ETF/섹터 정배열 상태", expanded=False):
        w = ns.get("weekly_alignment_df")
        if isinstance(w, pd.DataFrame): st.dataframe(w, use_container_width=True, hide_index=True)

    # 최종 52주 신고가 전략
    if isinstance(ns.get("summary"), pd.DataFrame):
        st.markdown("### 최종 52주 신고가 전략")
        st.caption("6M KOSPI 대비 RS Top12 ∩ 52주 신고가 → 최대 4종목 × 25% → MA10 종가 이탈 매도 · 편도 거래비용 0.10%")
        st.dataframe(ns["summary"], use_container_width=True, hide_index=True)
        if isinstance(ns.get("yearly"), pd.DataFrame):
            st.markdown("#### 연도별 수익률")
            st.dataframe(ns["yearly"], use_container_width=True)
        if isinstance(ns.get("stats_table"), pd.DataFrame):
            st.markdown("#### 거래 및 보유 통계")
            st.dataframe(ns["stats_table"], use_container_width=True, hide_index=True)
        if isinstance(ns.get("top12_df"), pd.DataFrame):
            st.markdown("#### 최신 6M RS Top12")
            st.dataframe(ns["top12_df"], use_container_width=True, hide_index=True)
        if isinstance(ns.get("latest_candidates"), pd.DataFrame):
            st.markdown("#### 오늘 신규 52주 신고가 후보")
            if ns["latest_candidates"].empty:
                st.info("오늘 신규 52주 신고가 후보가 없습니다.")
            else:
                st.dataframe(ns["latest_candidates"], use_container_width=True, hide_index=True)
        if isinstance(ns.get("strategy_live"), pd.Series) and isinstance(ns.get("kospi_live"), pd.Series):
            eq = pd.concat([ns["strategy_live"].rename("Strategy"), ns["kospi_live"].rename("KOSPI")], axis=1).dropna(how="all")
            st.markdown("#### 누적 성과")
            st.line_chart(eq, use_container_width=True)
        if isinstance(ns.get("strategy_dd"), pd.Series) and isinstance(ns.get("kospi_dd"), pd.Series):
            dd = pd.concat([ns["strategy_dd"].rename("Strategy DD"), ns["kospi_dd"].rename("KOSPI DD")], axis=1).dropna(how="all")
            st.markdown("#### Drawdown")
            st.line_chart(dd, use_container_width=True)
        with st.expander("최근 거래 / 보유 현황", expanded=False):
            if isinstance(ns.get("bt_trades"), pd.DataFrame):
                st.markdown("**최근 거래**")
                st.dataframe(ns["bt_trades"].tail(30), use_container_width=True, hide_index=True)
            if isinstance(ns.get("live_holdings"), pd.DataFrame):
                st.markdown("**최근 보유 현황**")
                st.dataframe(ns["live_holdings"].tail(30), use_container_width=True)
...
with st.sidebar:
    st.markdown("## 태린이아빠")
    st.caption("Market Dashboard · LIVE v11.2")
    st.markdown("---")
...
PAGE_GUIDES = {
    ...
    "한국 피어앤그리드 오실레이터": {
        ...
        "check": "Fear & Greed, EMA20, Oscillator와 함께 Elder Impulse·DeMark·개인 순매수 Capitulation을 봅니다. 극단값 하나보다 여러 보조지표가 같은 방향을 가리킬 때 의미를 더 크게 봅니다.",
    },
    "반도체 주가 모멘텀 점검": {
        "why": "AI·반도체 뉴스가 좋아도 실제 주가 상승이 일부 대형주에만 몰리면 추세의 질은 약할 수 있습니다. 반도체 밸류체인 전체의 Breadth와 모멘텀을 보면 상승이 넓게 퍼지는지 확인할 수 있습니다.",
        "check": "Breadth MA5, 5일 기울기, 60일 상승종목 비율과 신고가·지수 간 다이버전스를 같이 봅니다. 지수는 오르는데 Breadth가 약해지면 '쏠림 상승' 가능성을 점검합니다.",
    },
    "대만 월별 매출": {
    ...
    "일본 반도체": {
        "why": "일본은 대만처럼 기업별 월매출 공시가 통일돼 있지는 않지만 반도체 장비·전자부품·IC 생산과 장비수주·수출 통계가 잘 갖춰져 있습니다. 장비와 부품 사이클의 실제 물량 흐름을 확인하는 데 유용합니다.",
        "check": "생산지수·장비수주·장비수출이 전년동월 대비 개선되는지 확인합니다. 특히 수주가 먼저 움직이고 생산·수출이 뒤따르는지 보면 일본 반도체 장비 사이클의 확산 여부를 읽기 쉽습니다.",
    },
    "52주 신고가 전략 점검": {
        "why": "강한 종목은 상대강도가 높고 신고가를 갱신하며 추세를 이어가는 경우가 많습니다. 이 화면은 감으로 주도주를 고르지 않고 '상대강도 + 신고가 돌파 + 추세 유지' 규칙이 실제로 작동하는지 확인하기 위한 전략 점검 화면입니다.",
...
def run_korea_fear_greed(excel_bytes):
    """사용자 원본 한국 Fear & Greed 코드를 업로드 엑셀 바이트로 실행."""
    buf = io.StringIO()
    shown = []
    figs = []
    old_show = plt.show
    ...
    try:
        ...
        return True, ns, figs, shown, buf.getvalue(), None
    except Exception:
        return False, ns, figs, shown, buf.getvalue(), traceback.format_exc()
    finally:
        plt.show = old_show
...
if ACTIVE_PAGE == "52주 신고가 전략 점검":
    st.subheader("52주 신고가 전략 점검")
    ...
    if st.button("🔄 52주 신고가 + Rotation 최신 데이터 업데이트", key="upd_rotation", disabled=not IS_ADMIN):
        with st.spinner("52주 신고가 + Rotation 계산 중..."):
            time.sleep(2)
    ...
    if r is None:
        ...
        st.error("52주 신고가 + Rotation 계산 오류"); st.code(r[2][-12000:],language="text")

if ACTIVE_PAGE == "반도체 주가 모멘텀 점검":
    st.subheader("반도체 주가 모멘텀 점검")
    updated_caption("ai")
    if st.button("🔄 반도체 주가 모멘텀 최신 데이터 업데이트", key="upd_ai", disabled=not IS_ADMIN):
        with st.spinner("반도체 주가 모멘텀 Breadth 계산 중..."):
            time.sleep(2)
    ...
    if r is None:
        ...
        st.error("반도체 주가 모멘텀 계산 오류"); st.code(r[2][-12000:],language="text")
...
if ACTIVE_PAGE == "일본 반도체":
    ...
    if IS_ADMIN:
        st.info("등록·API 키 없는 Statistics Dashboard를 우선 사용하고, 상세 반도체 지표는 e-Stat 공개 표로 자동 fallback합니다. 업데이트 때만 숫자를 받아 GitHub에 압축 저장합니다.")
...
if ACTIVE_PAGE == "미국 ETF 소라티노 및 상대강도":
    ...
    st.caption("Yahoo Finance 기반 · SPY 대비 Mansfield RS · 6개월 상대수익률 · Sortino · Breadth · 과열 이격도 · 4·13·26·52주 정배열")
    ...
    if r is None:
        ...
        six=ns.get("six_month_summary")
        sortino=ns.get("sortino_ma10")
        hot=ns.get("sortino_ma10_gap247_p97_summary")
        full=ns.get("us_full_alignment_df")
        ma=ns.get("us_ma_alignment_df")
        if isinstance(mdf,pd.DataFrame):
            st.markdown("### Mansfield RS ≥ 70")
            st.dataframe(mdf, use_container_width=True, hide_index=True)
        if isinstance(six,pd.DataFrame):
            st.markdown("### 6개월 수익률 · SPY 대비 RS")
            st.dataframe(six.head(30), use_container_width=True)
        if isinstance(sortino,pd.DataFrame):
            st.markdown("### 6개월 Sortino 상위 20 · 10일선 이상")
            st.dataframe(sortino.tail(10), use_container_width=True)
        if isinstance(hot,pd.DataFrame):
            st.markdown("### Sortino 상위 ETF 과열도")
            st.dataframe(hot, use_container_width=True, hide_index=True)
        if isinstance(full,pd.DataFrame):
            st.markdown("### 완전 정배열 · Price > 4W > 13W > 26W > 52W")
            st.dataframe(full, use_container_width=True)
        if isinstance(ma,pd.DataFrame):
            st.markdown("### 이동평균 정배열 · 4W > 13W > 26W > 52W")
...
if ACTIVE_PAGE == "한국 ETF 소라티노 및 상대강도":
    ...
    st.caption("변동성 조정 모멘텀은 이평선 조건 없는 1개만 표시 · Sortino는 10일선/20일선/10주선 이상 3개를 표시합니다. 나머지 분석은 원본 로직을 유지합니다.")
    if IS_ADMIN:
            ...
            if st.button("▶ 업로드한 엑셀로 국내 주도업종 계산 · 저장", key="run_kr_sector"):
                with st.spinner("Mansfield RS · Momentum · Sortino · Breadth · 정배열 · 52주 전략까지 계산 중..."):
                    ...
                    if st.session_state.kr_sector_result[0]:
                        ...
                        st.success("계산 완료. 그래프 이미지는 저장하지 않고 숫자 결과만 압축 저장했습니다.")
    else:
...
if ACTIVE_PAGE == "한국 피어앤그리드 오실레이터":
    ...
    if IS_ADMIN:
        ...
        if kr_fg_file is not None:
            ...
            if st.button("▶ 업로드한 엑셀로 한국 피어앤그리드 계산", key="run_kr_fg"):
    ...
    if r is None:
        ...
        if not ok:
            ...
            c1, c2 = st.columns(2)
            for col, market, price_col in [(c1, "KOSPI", "코스피"), (c2, "KOSDAQ", "코스닥")]:
                df = ns.get(market.lower())
                with col:
                    st.subheader(market)
                    if isinstance(df, pd.DataFrame):
                        valid = df.dropna(subset=["Fear_Greed_Score","FG_EMA20_Score","Oscillator",price_col])
                        if not valid.empty:
                            last = valid.iloc[-1]
                            a,b,c = st.columns(3)
