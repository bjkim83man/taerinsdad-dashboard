...
/* 계산 상세 결과: 원본 줄바꿈은 st.code가 보존하고, 색상만 고정 */
...
def run_rotation():
    ...
    try:
        ...
        # v11.22: Weak Regime은 SPY와 QQQ에 모두 -1% 이하로 뒤질 때만 발동
        live_excess_spy = final_result.get(
            "Live_Excess_20D_SPY",
            final_result.get("Live_Excess_20D", np.nan),
        )
        live_excess_qqq = final_result.get("Live_Excess_20D_QQQ", np.nan)
        regime = final_result.get("Live_Regime", "")

        if pd.notna(live_excess_spy) and pd.notna(live_excess_qqq):
            weak_now = bool(
                float(live_excess_spy) <= -0.01
                and float(live_excess_qqq) <= -0.01
            )
            working = not weak_now
        ...
        payload = {
            ...
            # 하위호환용: 기존 live_excess_20d는 SPY 기준 유지
            "live_excess_20d": live_excess_spy,
            "live_excess_20d_spy": live_excess_spy,
            "live_excess_20d_qqq": live_excess_qqq,
            "regime": regime,
...
def render_rotation_simple(result):
    ...
    excess_spy = payload.get(
        "live_excess_20d_spy",
        payload.get("live_excess_20d", np.nan),
    )
    excess_qqq = payload.get("live_excess_20d_qqq", np.nan)
    ...
    if pd.notna(excess_spy) and pd.notna(excess_qqq):
        st.caption(
            f"최근 20거래일 순수 52W 전략 · "
            f"SPY 대비 {float(excess_spy):+.2%} · "
            f"QQQ 대비 {float(excess_qqq):+.2%} · "
            f"둘 다 -1% 이하일 때만 WEAK_ROTATION · 현재 Regime {regime}"
        )
    elif pd.notna(excess_spy):
        st.caption(
            f"최근 20거래일 순수 52W 전략의 SPY 대비 초과수익 {float(excess_spy):+.2%} "
            f"· QQQ 기준은 새 업데이트 후 표시됩니다. · 현재 Regime {regime}"
        )
    elif regime:
...
PAGE_GUIDES = {
    ...
    "미국 52주 신고가 전략 점검": {
        ...
        "check": "현재 Regime과 순수 52W 전략의 최근 20일 SPY·QQQ 대비 초과수익을 함께 봅니다. 두 기준지수 모두에 -1% 이하로 뒤질 때만 WEAK_ROTATION으로 전환합니다. 약세 Regime에서는 빈 자리에 Rotation 후보를 넣되 기존 보유종목은 매도 이동평균 이탈 전까지 유지합니다.",
    },
    "미국 ETF 소라티노 및 상대강도": {
